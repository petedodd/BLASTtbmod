...
// [[dust::param(popinit, has_default = FALSE, default_value = NULL, rank = 4, min = -Inf, max = Inf, integer = FALSE)]]
...
public:
  ...
  struct shared_type {
    ...
    int dim_raw_InfPP1;
    int dim_raw_InfPPj;
    int dim_raw_InfPPj_1;
    int dim_raw_InfPPj_2;
    int dim_raw_NotePP1;
    int dim_raw_NotePPj;
    int dim_raw_NotePPj_1;
    int dim_raw_NotePPj_2;
    int dim_Rdeaths;
    int dim_Rdeaths_1;
    int dim_Rdeaths_12;
    int dim_Rdeaths_2;
    ...
    std::vector<real_type> slfcr_rate_SC;
    real_type t_dur;
    std::vector<real_type> TB_HIV_mod;
    std::vector<real_type> TBd_rate;
    std::vector<real_type> TBd_rate_SC;
    real_type tfr;
    real_type tol;
  ...
  struct internal_type {
    ...
    std::vector<real_type> p_InfPP1;
    std::vector<real_type> p_InfPPj;
    ...
    std::vector<real_type> p_NotePP1;
    std::vector<real_type> p_NotePPj;
    ...
    std::vector<real_type> raw_InfPP1;
    std::vector<real_type> raw_InfPPj;
    std::vector<real_type> raw_NotePP1;
    std::vector<real_type> raw_NotePPj;
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_p_progress_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progress_2; ++j) {
        int k = 1;
        internal.p_progress[i - 1 + shared->dim_p_progress_1 * (j - 1) + shared->dim_p_progress_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + shared->progress_rate > shared->tol ? shared->progress_rate / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + shared->progress_rate) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progress_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progress_2; ++j) {
        int k = 2;
        internal.p_progress[i - 1 + shared->dim_p_progress_1 * (j - 1) + shared->dim_p_progress_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + shared->progress_rate > shared->tol ? shared->progress_rate / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + shared->progress_rate) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progress_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progress_2; ++j) {
        int k = 3;
        internal.p_progress[i - 1 + shared->dim_p_progress_1 * (j - 1) + shared->dim_p_progress_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + shared->progress_rate > shared->tol ? shared->progress_rate / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + shared->progress_rate) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_selfcr_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_selfcr_SC_2; ++j) {
        int k = 1;
        internal.p_selfcr_SC[i - 1 + shared->dim_p_selfcr_SC_1 * (j - 1) + shared->dim_p_selfcr_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + shared->slfcr_rate_SC[0] + shared->progress_rate > shared->tol ? shared->slfcr_rate_SC[0] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + shared->slfcr_rate_SC[0] + shared->progress_rate) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_selfcr_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_selfcr_SC_2; ++j) {
        int k = 2;
        internal.p_selfcr_SC[i - 1 + shared->dim_p_selfcr_SC_1 * (j - 1) + shared->dim_p_selfcr_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + shared->slfcr_rate_SC[1] + shared->progress_rate > shared->tol ? shared->slfcr_rate_SC[1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + shared->slfcr_rate_SC[1] + shared->progress_rate) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_selfcr_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_selfcr_SC_2; ++j) {
        int k = 3;
        internal.p_selfcr_SC[i - 1 + shared->dim_p_selfcr_SC_1 * (j - 1) + shared->dim_p_selfcr_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + shared->slfcr_rate_SC[2] + shared->progress_rate > shared->tol ? shared->slfcr_rate_SC[2] / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + shared->slfcr_rate_SC[2] + shared->progress_rate) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_TBdeaths_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_TBdeaths_SC_2; ++j) {
        int k = 1;
        internal.p_TBdeaths_SC[i - 1 + shared->dim_p_TBdeaths_SC_1 * (j - 1) + shared->dim_p_TBdeaths_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] > shared->tol ? shared->TBd_rate_SC[0] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_TBdeaths_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_TBdeaths_SC_2; ++j) {
        int k = 2;
        internal.p_TBdeaths_SC[i - 1 + shared->dim_p_TBdeaths_SC_1 * (j - 1) + shared->dim_p_TBdeaths_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] > shared->tol ? shared->TBd_rate_SC[1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_TBdeaths_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_TBdeaths_SC_2; ++j) {
        int k = 3;
        internal.p_TBdeaths_SC[i - 1 + shared->dim_p_TBdeaths_SC_1 * (j - 1) + shared->dim_p_TBdeaths_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] > shared->tol ? shared->TBd_rate_SC[2] / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2]) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_detect_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_detect_SC_2; ++j) {
        int k = 1;
        internal.p_detect_SC[i - 1 + shared->dim_p_detect_SC_1 * (j - 1) + shared->dim_p_detect_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 0 + i - 1] + shared->slfcr_rate_SC[0] + shared->progress_rate > shared->tol ? internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 0 + i - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 0 + i - 1] + shared->slfcr_rate_SC[0] + shared->progress_rate) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_detect_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_detect_SC_2; ++j) {
        int k = 2;
        internal.p_detect_SC[i - 1 + shared->dim_p_detect_SC_1 * (j - 1) + shared->dim_p_detect_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 1 + i - 1] + shared->slfcr_rate_SC[1] + shared->progress_rate > shared->tol ? internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 1 + i - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 1 + i - 1] + shared->slfcr_rate_SC[1] + shared->progress_rate) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_detect_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_p_detect_SC_2; ++j) {
        int k = 3;
        internal.p_detect_SC[i - 1 + shared->dim_p_detect_SC_1 * (j - 1) + shared->dim_p_detect_SC_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 2 + i - 1] + shared->slfcr_rate_SC[2] + shared->progress_rate > shared->tol ? internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 2 + i - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 2 + i - 1] + shared->slfcr_rate_SC[2] + shared->progress_rate) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_SCHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_SCHIV_2; ++j) {
        int k = 1;
        internal.p_SCHIV[i - 1 + shared->dim_p_SCHIV_1 * (j - 1) + shared->dim_p_SCHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 0 + i - 1] + shared->slfcr_rate_SC[0] + shared->progress_rate + internal.HIV_rate_yr[j - 1] > shared->tol ? internal.HIV_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 0 + i - 1] + shared->slfcr_rate_SC[0] + shared->progress_rate + internal.HIV_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_SCHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_SCHIV_2; ++j) {
        int k = 2;
        internal.p_SCHIV[i - 1 + shared->dim_p_SCHIV_1 * (j - 1) + shared->dim_p_SCHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 1 + i - 1] + shared->slfcr_rate_SC[1] + shared->progress_rate + internal.ART_rate_yr[j - 1] > shared->tol ? internal.ART_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 1 + i - 1] + shared->slfcr_rate_SC[1] + shared->progress_rate + internal.ART_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_SCHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_SCHIV_2; ++j) {
    ...
    for (int i = 1; i <= shared->dim_rate_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_SC_2; ++j) {
        int k = 1;
        internal.rate_SC[i - 1 + shared->dim_rate_SC_1 * (j - 1) + shared->dim_rate_SC_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->TBd_rate_SC[0] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 0 + i - 1] + shared->slfcr_rate_SC[0] + shared->progress_rate + shared->age_rate[j - 1] + internal.HIV_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_SC_2; ++j) {
        int k = 2;
        internal.rate_SC[i - 1 + shared->dim_rate_SC_1 * (j - 1) + shared->dim_rate_SC_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->TBd_rate_SC[1] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 1 + i - 1] + shared->slfcr_rate_SC[1] + shared->progress_rate + shared->age_rate[j - 1] + internal.ART_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_SC_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_SC_2; ++j) {
        int k = 3;
        internal.rate_SC[i - 1 + shared->dim_rate_SC_1 * (j - 1) + shared->dim_rate_SC_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->TBd_rate_SC[2] + internal.dtct_rate_SCm[shared->dim_dtct_rate_SCm_1 * 2 + i - 1] + shared->slfcr_rate_SC[2] + shared->progress_rate + shared->age_rate[j - 1];
      }
    }
    for (int i = 1; i <= shared->patch_dims; ++i) {
      for (int j = 2; j <= shared->patch_dims; ++j) {
        internal.raw_InfPPj[i - 1 + shared->dim_raw_InfPPj_1 * (j - 1)] = internal.foitemp[shared->dim_foitemp_1 * (j - 1) + i - 1] / (real_type) (odin_sum2<real_type>(internal.foitemp.data(), i - 1, i, j - 1, shared->patch_dims, shared->dim_foitemp_1) + shared->tol);
      }
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      for (int j = 2; j <= shared->patch_dims; ++j) {
        internal.p_InfPPj[i - 1 + shared->dim_p_InfPPj_1 * (j - 1)] = (internal.raw_InfPPj[shared->dim_raw_InfPPj_1 * (j - 1) + i - 1] < 0 ? 0 : ((internal.raw_InfPPj[shared->dim_raw_InfPPj_1 * (j - 1) + i - 1] > 1 ? 1 : internal.raw_InfPPj[shared->dim_raw_InfPPj_1 * (j - 1) + i - 1])));
      }
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      internal.raw_InfPP1[i - 1] = internal.foitemp[shared->dim_foitemp_1 * 0 + i - 1] / (real_type) (internal.foi[i - 1] + shared->tol);
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      internal.p_InfPP1[i - 1] = (internal.raw_InfPP1[i - 1] < 0 ? 0 : ((internal.raw_InfPP1[i - 1] > 1 ? 1 : internal.raw_InfPP1[i - 1])));
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      internal.raw_NotePP1[i - 1] = internal.ellij[shared->dim_ellij_1 * 0 + i - 1] / (real_type) (odin_sum2<real_type>(internal.ellij.data(), i - 1, i, 0, shared->patch_dims, shared->dim_ellij_1) + shared->tol);
    }
    for (int i = 1; i <= shared->patch_dims; ++i) {
      for (int j = 2; j <= shared->patch_dims; ++j) {
        internal.raw_NotePPj[i - 1 + shared->dim_raw_NotePPj_1 * (j - 1)] = internal.ellij[shared->dim_ellij_1 * (j - 1) + i - 1] / (real_type) (odin_sum2<real_type>(internal.ellij.data(), i - 1, i, j - 1, shared->patch_dims, shared->dim_ellij_1) + shared->tol);
      }
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      internal.p_NotePP1[i - 1] = (internal.raw_NotePP1[i - 1] < 0 ? 0 : ((internal.raw_NotePP1[i - 1] > 1 ? 1 : internal.raw_NotePP1[i - 1])));
    }
    for (int i = 1; i <= shared->patch_dims; ++i) {
      for (int j = 2; j <= shared->patch_dims; ++j) {
        internal.p_NotePPj[i - 1 + shared->dim_p_NotePPj_1 * (j - 1)] = (internal.raw_NotePPj[shared->dim_raw_NotePPj_1 * (j - 1) + i - 1] < 0 ? 0 : ((internal.raw_NotePPj[shared->dim_raw_NotePPj_1 * (j - 1) + i - 1] > 1 ? 1 : internal.raw_NotePPj[shared->dim_raw_NotePPj_1 * (j - 1) + i - 1])));
      }
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      int j = 1;
      internal.NotesByPatchPatch[i - 1 + shared->dim_NotesByPatchPatch_1 * (j - 1)] = (internal.NotesByPatch[i - 1] > 0 ? dust::random::binomial<real_type>(rng_state, internal.NotesByPatch[i - 1], internal.p_NotePP1[i - 1]) : 0);
    }
    for (int i = 1; i <= shared->patch_dims; ++i) {
      for (int j = 2; j <= shared->patch_dims; ++j) {
        internal.NotesByPatchPatch[i - 1 + shared->dim_NotesByPatchPatch_1 * (j - 1)] = (internal.NotesByPatch[i - 1] - odin_sum2<real_type>(internal.NotesByPatchPatch.data(), i - 1, i, 0, j - 1, shared->dim_NotesByPatchPatch_1) > 0 ? dust::random::binomial<real_type>(rng_state, internal.NotesByPatch[i - 1] - odin_sum2<real_type>(internal.NotesByPatchPatch.data(), i - 1, i, 0, j - 1, shared->dim_NotesByPatchPatch_1), internal.p_NotePPj[shared->dim_p_NotePPj_1 * (j - 1) + i - 1]) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->patch_dims; ++i) {
      int j = 1;
      internal.InfsByPatchPatch[i - 1 + shared->dim_InfsByPatchPatch_1 * (j - 1)] = (internal.InfsByPatch[i - 1] > 0 ? dust::random::binomial<real_type>(rng_state, internal.InfsByPatch[i - 1], internal.p_InfPP1[i - 1]) : 0);
    }
    for (int i = 1; i <= shared->patch_dims; ++i) {
      for (int j = 2; j <= shared->patch_dims; ++j) {
        internal.InfsByPatchPatch[i - 1 + shared->dim_InfsByPatchPatch_1 * (j - 1)] = (internal.InfsByPatch[i - 1] - odin_sum2<real_type>(internal.InfsByPatchPatch.data(), i - 1, i, 0, j - 1, shared->dim_InfsByPatchPatch_1) > 0 ? dust::random::binomial<real_type>(rng_state, internal.InfsByPatch[i - 1] - odin_sum2<real_type>(internal.InfsByPatchPatch.data(), i - 1, i, 0, j - 1, shared->dim_InfsByPatchPatch_1), internal.p_InfPPj[shared->dim_p_InfPPj_1 * (j - 1) + i - 1]) : 0);
      }
    }
...
dust::pars_type<stocm> dust_pars<stocm>(cpp11::list user) {
  ...
  shared->dim_slfcr_rate_SC = 3;
  shared->dim_TBd_rate = 3;
  shared->dim_TBd_rate_SC = 3;
  shared->dim_zk = 6;
  ...
  shared->slfcr_rate_SC = std::vector<real_type>(shared->dim_slfcr_rate_SC);
  ...
  shared->dim_p_InfPP1 = shared->patch_dims;
  shared->dim_p_InfPPj_1 = shared->patch_dims;
  shared->dim_p_InfPPj_2 = shared->patch_dims;
  ...
  shared->dim_p_NotePP1 = shared->patch_dims;
  shared->dim_p_NotePPj_1 = shared->patch_dims;
  shared->dim_p_NotePPj_2 = shared->patch_dims;
  ...
  shared->dim_popinit_1 = 7;
  shared->dim_popinit_2 = shared->patch_dims;
  shared->dim_popinit_3 = shared->age_dims;
  shared->dim_popinit_4 = shared->HIV_dims;
  ...
  shared->dim_raw_InfPP1 = shared->patch_dims;
  shared->dim_raw_InfPPj_1 = shared->patch_dims;
  shared->dim_raw_InfPPj_2 = shared->patch_dims;
  shared->dim_raw_NotePP1 = shared->patch_dims;
  shared->dim_raw_NotePPj_1 = shared->patch_dims;
  shared->dim_raw_NotePPj_2 = shared->patch_dims;
  ...
  internal.p_InfPP1 = std::vector<real_type>(shared->dim_p_InfPP1);
  internal.p_NotePP1 = std::vector<real_type>(shared->dim_p_NotePP1);
  internal.ppop = std::vector<real_type>(shared->dim_ppop);
  internal.raw_InfPP1 = std::vector<real_type>(shared->dim_raw_InfPP1);
  internal.raw_NotePP1 = std::vector<real_type>(shared->dim_raw_NotePP1);
  ...
  shared->dim_p_InfPPj = shared->dim_p_InfPPj_1 * shared->dim_p_InfPPj_2;
  ...
  shared->dim_p_NotePPj = shared->dim_p_NotePPj_1 * shared->dim_p_NotePPj_2;
  ...
  shared->dim_popinit = shared->dim_popinit_1 * shared->dim_popinit_2 * shared->dim_popinit_3 * shared->dim_popinit_4;
  shared->dim_popinit_12 = shared->dim_popinit_1 * shared->dim_popinit_2;
  shared->dim_popinit_123 = shared->dim_popinit_1 * shared->dim_popinit_2 * shared->dim_popinit_3;
  ...
  shared->dim_raw_InfPPj = shared->dim_raw_InfPPj_1 * shared->dim_raw_InfPPj_2;
  shared->dim_raw_NotePPj = shared->dim_raw_NotePPj_1 * shared->dim_raw_NotePPj_2;
  ...
  {
     int i = 2;
     shared->slfcr_rate[i - 1] = shared->HIV_dur_ratio * (1 - shared->cfr) / (real_type) shared->dur;
  }
  {
     int i = 3;
     shared->slfcr_rate[i - 1] = (1 - shared->cfr) / (real_type) shared->dur;
  ...
  internal.p_InfPPj = std::vector<real_type>(shared->dim_p_InfPPj);
  ...
  internal.p_NotePPj = std::vector<real_type>(shared->dim_p_NotePPj);
  ...
  internal.raw_InfPPj = std::vector<real_type>(shared->dim_raw_InfPPj);
  internal.raw_NotePPj = std::vector<real_type>(shared->dim_raw_NotePPj);
  ...
  shared->popinit = user_get_array_fixed<real_type, 4>(user, "popinit", shared->popinit, {shared->dim_popinit_1, shared->dim_popinit_2, shared->dim_popinit_3, shared->dim_popinit_4}, NA_REAL, NA_REAL);
  shared->ppC = shared->ppB + shared->Palpha;
  for (int i = 1; i <= shared->dim_D_1; ++i) {
    for (int j = 1; j <= shared->dim_D_2; ++j) {
      for (int k = 1; k <= shared->dim_D_3; ++k) {
        shared->initial_D[i - 1 + shared->dim_D_1 * (j - 1) + shared->dim_D_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 3];
      }
    }
  }
  for (int i = 1; i <= shared->dim_LL_1; ++i) {
    for (int j = 1; j <= shared->dim_LL_2; ++j) {
      for (int k = 1; k <= shared->dim_LL_3; ++k) {
        shared->initial_LL[i - 1 + shared->dim_LL_1 * (j - 1) + shared->dim_LL_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 2];
      }
    }
  }
  for (int i = 1; i <= shared->dim_LR_1; ++i) {
    for (int j = 1; j <= shared->dim_LR_2; ++j) {
      for (int k = 1; k <= shared->dim_LR_3; ++k) {
        shared->initial_LR[i - 1 + shared->dim_LR_1 * (j - 1) + shared->dim_LR_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 1];
      }
    }
  }
  for (int i = 1; i <= shared->dim_N_1; ++i) {
    for (int j = 1; j <= shared->dim_N_2; ++j) {
      for (int k = 1; k <= shared->dim_N_3; ++k) {
        shared->initial_N[i - 1 + shared->dim_N_1 * (j - 1) + shared->dim_N_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 0] + shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 1] + shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 2] + shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 3] + shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 4] + shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 5] + shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 6];
      }
    }
  }
  for (int i = 1; i <= shared->dim_R_1; ++i) {
    for (int j = 1; j <= shared->dim_R_2; ++j) {
      for (int k = 1; k <= shared->dim_R_3; ++k) {
        shared->initial_R[i - 1 + shared->dim_R_1 * (j - 1) + shared->dim_R_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 6];
      }
    }
  }
  for (int i = 1; i <= shared->dim_SC_1; ++i) {
    for (int j = 1; j <= shared->dim_SC_2; ++j) {
      for (int k = 1; k <= shared->dim_SC_3; ++k) {
        shared->initial_SC[i - 1 + shared->dim_SC_1 * (j - 1) + shared->dim_SC_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 4];
      }
    }
  }
  for (int i = 1; i <= shared->dim_Tr_1; ++i) {
    for (int j = 1; j <= shared->dim_Tr_2; ++j) {
      for (int k = 1; k <= shared->dim_Tr_3; ++k) {
        shared->initial_Tr[i - 1 + shared->dim_Tr_1 * (j - 1) + shared->dim_Tr_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 5];
      }
    }
  }
  for (int i = 1; i <= shared->dim_U_1; ++i) {
    for (int j = 1; j <= shared->dim_U_2; ++j) {
      for (int k = 1; k <= shared->dim_U_3; ++k) {
        shared->initial_U[i - 1 + shared->dim_U_1 * (j - 1) + shared->dim_U_12 * (k - 1)] = shared->popinit[shared->dim_popinit_123 * (k - 1) + shared->dim_popinit_12 * (j - 1) + shared->dim_popinit_1 * (i - 1) + 0];
      }
    }
  }
