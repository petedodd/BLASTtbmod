...
public:
  ...
  size_t size() const {
    return shared->dim_bg_deaths + shared->dim_D + shared->dim_incidence + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_notes + shared->dim_notifrate + shared->dim_R + shared->dim_SC + shared->dim_TB_deaths + shared->dim_Tr + shared->dim_U + 2;
  }
  std::vector<real_type> initial(size_t step, rng_state_type& rng_state) {
    std::vector<real_type> state(shared->dim_bg_deaths + shared->dim_D + shared->dim_incidence + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_notes + shared->dim_notifrate + shared->dim_R + shared->dim_SC + shared->dim_TB_deaths + shared->dim_Tr + shared->dim_U + 2);
    ...
    std::copy(internal.initial_U.begin(), internal.initial_U.end(), state.begin() + 2);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_foitemp_1; ++i) {
      for (int j = 1; j <= shared->dim_foitemp_2; ++j) {
        internal.foitemp[i - 1 + shared->dim_foitemp_1 * (j - 1)] = beta_seas * shared->MM[shared->dim_MM_1 * (j - 1) + i - 1] * shared->relinf * (odin_sum3<real_type>(D, i - 1, i, 0, shared->dim_D_2, 0, shared->dim_D_3, shared->dim_D_1, shared->dim_D_12) + odin_sum3<real_type>(SC, i - 1, i, 0, shared->dim_SC_2, 0, shared->dim_SC_3, shared->dim_SC_1, shared->dim_SC_12)) / (real_type) (odin_sum3<real_type>(N, i - 1, i, 0, shared->dim_N_2, 0, shared->dim_N_3, shared->dim_N_1, shared->dim_N_12) + static_cast<real_type>(1.0000000000000001e-15));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_LRHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LRHIV_2; ++j) {
        int k = 1;
        internal.p_LRHIV[i - 1 + shared->dim_p_LRHIV_1 * (j - 1) + shared->dim_p_LRHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + internal.HIV_rate_yr[j - 1] > shared->tol ? internal.HIV_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + internal.HIV_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_LRHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LRHIV_2; ++j) {
        int k = 2;
        internal.p_LRHIV[i - 1 + shared->dim_p_LRHIV_1 * (j - 1) + shared->dim_p_LRHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + internal.ART_rate_yr[j - 1] > shared->tol ? internal.ART_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + internal.ART_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_LRHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LRHIV_2; ++j) {
    ...
    for (int i = 1; i <= shared->dim_p_progFast_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progFast_2; ++j) {
        int k = 1;
        internal.p_progFast[i - 1 + shared->dim_p_progFast_1 * (j - 1) + shared->dim_p_progFast_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL > shared->tol ? shared->IRR[i - 1] * shared->pDf / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progFast_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progFast_2; ++j) {
        int k = 2;
        internal.p_progFast[i - 1 + shared->dim_p_progFast_1 * (j - 1) + shared->dim_p_progFast_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL > shared->tol ? shared->IRR[i - 1] * shared->pDf / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progFast_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progFast_2; ++j) {
        int k = 3;
        internal.p_progFast[i - 1 + shared->dim_p_progFast_1 * (j - 1) + shared->dim_p_progFast_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL > shared->tol ? shared->IRR[i - 1] * shared->pDf / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_relapse_1; ++i) {
      for (int j = 1; j <= shared->dim_p_relapse_2; ++j) {
        int k = 1;
        internal.p_relapse[i - 1 + shared->dim_p_relapse_1 * (j - 1) + shared->dim_p_relapse_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDr * shared->IRR[i - 1] > shared->tol ? shared->IRR[i - 1] * shared->pDr / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDr * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_relapse_1; ++i) {
      for (int j = 1; j <= shared->dim_p_relapse_2; ++j) {
        int k = 2;
        internal.p_relapse[i - 1 + shared->dim_p_relapse_1 * (j - 1) + shared->dim_p_relapse_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDr * shared->IRR[i - 1] > shared->tol ? shared->IRR[i - 1] * shared->pDr / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDr * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_relapse_1; ++i) {
      for (int j = 1; j <= shared->dim_p_relapse_2; ++j) {
        int k = 3;
        internal.p_relapse[i - 1 + shared->dim_p_relapse_1 * (j - 1) + shared->dim_p_relapse_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->pDr * shared->IRR[i - 1] > shared->tol ? shared->IRR[i - 1] * shared->pDr / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->pDr * shared->IRR[i - 1]) : 0);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_rate_LR_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_LR_2; ++j) {
        int k = 1;
        internal.rate_LR[i - 1 + shared->dim_rate_LR_1 * (j - 1) + shared->dim_rate_LR_12 * (k - 1)] = internal.mu_noHIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + shared->age_rate[j - 1] + internal.HIV_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_LR_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_LR_2; ++j) {
        int k = 2;
        internal.rate_LR[i - 1 + shared->dim_rate_LR_1 * (j - 1) + shared->dim_rate_LR_12 * (k - 1)] = internal.mu_HIV_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + shared->age_rate[j - 1] + internal.ART_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_LR_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_LR_2; ++j) {
        int k = 3;
        internal.rate_LR[i - 1 + shared->dim_rate_LR_1 * (j - 1) + shared->dim_rate_LR_12 * (k - 1)] = internal.mu_ART_t[j - 1] + shared->pDf * shared->IRR[i - 1] + shared->pLL + shared->age_rate[j - 1];
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_LLHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LLHIV_2; ++j) {
        int k = 1;
        internal.p_LLHIV[i - 1 + shared->dim_p_LLHIV_1 * (j - 1) + shared->dim_p_LLHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDs * shared->IRR[i - 1] + internal.HIV_rate_yr[j - 1] > shared->tol ? internal.HIV_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDs * shared->IRR[i - 1] + internal.HIV_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_LLHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LLHIV_2; ++j) {
        int k = 2;
        internal.p_LLHIV[i - 1 + shared->dim_p_LLHIV_1 * (j - 1) + shared->dim_p_LLHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDs * shared->IRR[i - 1] + internal.ART_rate_yr[j - 1] > shared->tol ? internal.ART_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDs * shared->IRR[i - 1] + internal.ART_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_LLHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LLHIV_2; ++j) {
    ...
    for (int i = 1; i <= shared->dim_p_LLinfs_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LLinfs_2; ++j) {
        int k = 1;
        internal.p_LLinfs[i - 1 + shared->dim_p_LLinfs_1 * (j - 1) + shared->dim_p_LLinfs_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDs * shared->IRR[i - 1] > shared->tol ? shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDs * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_LLinfs_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LLinfs_2; ++j) {
        int k = 2;
        internal.p_LLinfs[i - 1 + shared->dim_p_LLinfs_1 * (j - 1) + shared->dim_p_LLinfs_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDs * shared->IRR[i - 1] > shared->tol ? shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDs * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_LLinfs_1; ++i) {
      for (int j = 1; j <= shared->dim_p_LLinfs_2; ++j) {
        int k = 3;
        internal.p_LLinfs[i - 1 + shared->dim_p_LLinfs_1 * (j - 1) + shared->dim_p_LLinfs_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[2] + shared->pDs * shared->IRR[i - 1] > shared->tol ? shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[2] / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[2] + shared->pDs * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progSlow_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progSlow_2; ++j) {
        int k = 1;
        internal.p_progSlow[i - 1 + shared->dim_p_progSlow_1 * (j - 1) + shared->dim_p_progSlow_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->pDs * shared->IRR[i - 1] > shared->tol ? shared->IRR[i - 1] * shared->pDs / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDs * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progSlow_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progSlow_2; ++j) {
        int k = 2;
        internal.p_progSlow[i - 1 + shared->dim_p_progSlow_1 * (j - 1) + shared->dim_p_progSlow_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->pDs * shared->IRR[i - 1] > shared->tol ? shared->IRR[i - 1] * shared->pDs / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDs * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_progSlow_1; ++i) {
      for (int j = 1; j <= shared->dim_p_progSlow_2; ++j) {
        int k = 3;
        internal.p_progSlow[i - 1 + shared->dim_p_progSlow_1 * (j - 1) + shared->dim_p_progSlow_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->pDs * shared->IRR[i - 1] > shared->tol ? shared->IRR[i - 1] * shared->pDs / (real_type) (internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[2] + shared->pDs * shared->IRR[i - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_RHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_RHIV_2; ++j) {
        int k = 1;
        internal.p_RHIV[i - 1 + shared->dim_p_RHIV_1 * (j - 1) + shared->dim_p_RHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDr * shared->IRR[i - 1] + internal.HIV_rate_yr[j - 1] > shared->tol ? internal.HIV_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDr * shared->IRR[i - 1] + internal.HIV_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_RHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_RHIV_2; ++j) {
        int k = 2;
        internal.p_RHIV[i - 1 + shared->dim_p_RHIV_1 * (j - 1) + shared->dim_p_RHIV_12 * (k - 1)] = (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDr * shared->IRR[i - 1] + internal.ART_rate_yr[j - 1] > shared->tol ? internal.ART_rate_yr[j - 1] / (real_type) (internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDr * shared->IRR[i - 1] + internal.ART_rate_yr[j - 1]) : 0);
      }
    }
    for (int i = 1; i <= shared->dim_p_RHIV_1; ++i) {
      for (int j = 1; j <= shared->dim_p_RHIV_2; ++j) {
    ...
    for (int i = 1; i <= shared->dim_rate_LL_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_LL_2; ++j) {
        int k = 1;
        internal.rate_LL[i - 1 + shared->dim_rate_LL_1 * (j - 1) + shared->dim_rate_LL_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDs * shared->IRR[i - 1] + shared->age_rate[j - 1] + internal.HIV_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_LL_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_LL_2; ++j) {
        int k = 2;
        internal.rate_LL[i - 1 + shared->dim_rate_LL_1 * (j - 1) + shared->dim_rate_LL_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDs * shared->IRR[i - 1] + shared->age_rate[j - 1] + internal.ART_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_LL_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_LL_2; ++j) {
        int k = 3;
        internal.rate_LL[i - 1 + shared->dim_rate_LL_1 * (j - 1) + shared->dim_rate_LL_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[2] + shared->pDs * shared->IRR[i - 1] + shared->age_rate[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_R_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_R_2; ++j) {
        int k = 1;
        internal.rate_R[i - 1 + shared->dim_rate_R_1 * (j - 1) + shared->dim_rate_R_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_noHIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[0] + shared->pDr * shared->IRR[i - 1] + shared->age_rate[j - 1] + internal.HIV_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_R_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_R_2; ++j) {
        int k = 2;
        internal.rate_R[i - 1 + shared->dim_rate_R_1 * (j - 1) + shared->dim_rate_R_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_HIV_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[1] + shared->pDr * shared->IRR[i - 1] + shared->age_rate[j - 1] + internal.ART_rate_yr[j - 1];
      }
    }
    for (int i = 1; i <= shared->dim_rate_R_1; ++i) {
      for (int j = 1; j <= shared->dim_rate_R_2; ++j) {
        int k = 3;
        internal.rate_R[i - 1 + shared->dim_rate_R_1 * (j - 1) + shared->dim_rate_R_12 * (k - 1)] = internal.m_in_t[j - 1] + internal.mu_ART_t[j - 1] + shared->v * internal.foi[i - 1] * shared->TB_HIV_mod[2] + shared->pDr * shared->IRR[i - 1] + shared->age_rate[j - 1];
      }
    }
...
dust::pars_type<stocm> dust_pars<stocm>(cpp11::list user) {
  ...
  shared->offset_variable_bg_deaths = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_notes + shared->dim_notifrate + shared->dim_R + shared->dim_SC + shared->dim_TB_deaths + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_D = shared->dim_LL + shared->dim_LR + shared->dim_U + 2;
  shared->offset_variable_incidence = shared->dim_bg_deaths + shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_notes + shared->dim_notifrate + shared->dim_R + shared->dim_SC + shared->dim_TB_deaths + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_LL = shared->dim_LR + shared->dim_U + 2;
  shared->offset_variable_LR = shared->dim_U + 2;
  shared->offset_variable_N = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_R + shared->dim_SC + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_notes = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_notifrate + shared->dim_R + shared->dim_SC + shared->dim_TB_deaths + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_notifrate = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_R + shared->dim_SC + shared->dim_TB_deaths + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_R = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_SC + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_SC = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_U + 2;
  shared->offset_variable_TB_deaths = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_N + shared->dim_R + shared->dim_SC + shared->dim_Tr + shared->dim_U + 2;
  shared->offset_variable_Tr = shared->dim_D + shared->dim_LL + shared->dim_LR + shared->dim_SC + shared->dim_U + 2;
...
cpp11::sexp dust_info<stocm>(const dust::pars_type<stocm>& pars) {
  ...
  cpp11::writable::strings nms({"tot_incidence", "beta_test", "U", "LR", "LL", "D", "SC", "Tr", "R", "N", "TB_deaths", "notifrate", "notes", "bg_deaths", "incidence"});
  cpp11::writable::list dim(15);
  dim[0] = cpp11::writable::integers({1});
  dim[1] = cpp11::writable::integers({1});
  dim[2] = cpp11::writable::integers({shared->dim_U_1, shared->dim_U_2, shared->dim_U_3});
  dim[3] = cpp11::writable::integers({shared->dim_LR_1, shared->dim_LR_2, shared->dim_LR_3});
  dim[4] = cpp11::writable::integers({shared->dim_LL_1, shared->dim_LL_2, shared->dim_LL_3});
  dim[5] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2, shared->dim_D_3});
  dim[6] = cpp11::writable::integers({shared->dim_SC_1, shared->dim_SC_2, shared->dim_SC_3});
  dim[7] = cpp11::writable::integers({shared->dim_Tr_1, shared->dim_Tr_2, shared->dim_Tr_3});
  dim[8] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[9] = cpp11::writable::integers({shared->dim_N_1, shared->dim_N_2, shared->dim_N_3});
  dim[10] = cpp11::writable::integers({shared->dim_TB_deaths_1, shared->dim_TB_deaths_2, shared->dim_TB_deaths_3});
  dim[11] = cpp11::writable::integers({shared->dim_notifrate_1, shared->dim_notifrate_2, shared->dim_notifrate_3});
  dim[12] = cpp11::writable::integers({shared->dim_notes_1, shared->dim_notes_2, shared->dim_notes_3});
  dim[13] = cpp11::writable::integers({shared->dim_bg_deaths_1, shared->dim_bg_deaths_2, shared->dim_bg_deaths_3});
  dim[14] = cpp11::writable::integers({shared->dim_incidence_1, shared->dim_incidence_2, shared->dim_incidence_3});
  dim.names() = nms;
  cpp11::writable::list index(15);
  index[0] = cpp11::writable::integers({1});
  index[1] = cpp11::writable::integers({2});
  index[2] = integer_sequence(3, shared->dim_U);
  index[3] = integer_sequence(shared->offset_variable_LR + 1, shared->dim_LR);
  index[4] = integer_sequence(shared->offset_variable_LL + 1, shared->dim_LL);
  index[5] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[6] = integer_sequence(shared->offset_variable_SC + 1, shared->dim_SC);
  index[7] = integer_sequence(shared->offset_variable_Tr + 1, shared->dim_Tr);
  index[8] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[9] = integer_sequence(shared->offset_variable_N + 1, shared->dim_N);
  index[10] = integer_sequence(shared->offset_variable_TB_deaths + 1, shared->dim_TB_deaths);
  index[11] = integer_sequence(shared->offset_variable_notifrate + 1, shared->dim_notifrate);
  index[12] = integer_sequence(shared->offset_variable_notes + 1, shared->dim_notes);
  index[13] = integer_sequence(shared->offset_variable_bg_deaths + 1, shared->dim_bg_deaths);
